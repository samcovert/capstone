import './Homepage.css'

const Homepage = () => {
    return (
        <div className="homepage-container">
        <h1 className="home-title">Gone But Never Forgotten</h1>
        <h4 className="home-sub">In memory of the Phoenix/Arizona Coyotes</h4>
        <p className="home-description">Yotes 4 Ever is dedicated to keeping the memory of the
            Phoenix/Arizona Coyotes alive. Click the profile button to create a profile so you can join
             the Coyotes Community in buying/selling items, sharing news, and memories of our beloved Desert Dawgs.
        </p>
        </div>
    )
}

export default Homepage
